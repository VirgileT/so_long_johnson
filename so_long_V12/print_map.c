...
/*   Updated: 2023/02/17 19:47:16 by vtestut          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	mlx_put_image_to_window(vars->mlx, vars->win, sprite, x * IMGSIZE, y * IMGSIZE);
}
...
void ft_display_game(t_vars *vars)
{
	...
	vars->win = mlx_new_window(vars->mlx, winwidht, winheight, "so_long_johnson");
	if(vars->win == NULL)
	{
		free(vars->mlx);
		return;
